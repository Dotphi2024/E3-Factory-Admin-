...
@section('main_content')
    ...
    <form class="row g-3 needs-validation" action="{{ route('master.video-gallery.update', $video_gallery->id) }}" method="POST"
                        ...
                        <div class="p-4 border rounded">
                            ...
                            <div class="row g-3 needs-validation">
                                <div class="col-md-6">
                                    ...
                                    <input type="text" class="form-control" id="title" value="{{ $video_gallery->title }}"
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input type="text" class="form-control" id="video_url" name="video_url" placeholder="Enter Url" value="{{ $video_gallery->video_url }}" required>
                                </div>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <select name="visibility" id="visibility" class="form-select" required>
                                        ...
                                        <option value="Guest" {{ $video_gallery->visibility == 'Guest' ? 'selected' : ''}}>Guest</option>
                                        <option value="Batch" {{ $video_gallery->visibility == 'Batch' ? 'selected' : ''}}>Batch</option>
                                    </select>
                                </div>
                                <div class="col-md-6" id="batch-id-div" style="display: none">
                                    ...
                                    <select name="batch_id" id="batch_id" class="single-select">
                                        ...
                                        @foreach ($batches as $batch)
                                            <option value="{{ $batch->id }}" {{ $batch->id == $video_gallery->batch_id ? 'selected' : ''}}>{{ $batch->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
