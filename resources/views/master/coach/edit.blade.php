...
@section('main_content')
    ...
    <div class="page-breadcrumb d-none d-sm-flex align-items-center mb-3">
        <div class="breadcrumb-title pe-3">Edit Coach</div>
        <div class="ps-3">
    ...
    <form class="row g-3 needs-validation" action="{{ route('master.coaches.update', $coach->id) }}" method="POST" autocomplete="off"
                        ...
                        <div class="p-4 border rounded">
                            ...
                            <div class="row g-3 needs-validation">
                                <div class="col-md-6">
                                    ...
                                    <select name="batch_id" id="batch_id" class="" data-placeholder="Select Batch" required>
                                        <option label="Select"></option>
                                        @foreach ($batches as $batch)
                                            <option value="{{ $batch->id }}" {{ $coach->batch_id == $batch->id ? 'selected' : '' }}>{{ $batch->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="col-md-6">
                                    ...
                                    <select name="participant_id" id="participant_id" class="" required>
                                        ...
                                        @foreach ($participants as $participant)
                                            <option value="{{ $participant->id }}" {{ $coach->participant_id == $participant->id ? 'selected' : '' }}>{{ $participant->first_name }}
