...
@section('main_content')
    <h6 class="mb-0 text-uppercase">Edit Roles now</h6>
    <hr />
            ...
            <div class="card">
                <div class="card-body">
                    <div class="border p-3 rounded">
                        <form class="row g-3" action="{{ route('master.roles.update', $role->id) }}" method="POST">
                            @csrf
                            <div class="col-md-12">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="d-flex">
                                                ...
                                                <input type="checkbox" name="{{ $row . '_list' }}" value="1"
                                                    class="form-check" {{ $role->hasPermissionTo($row . ' list') ? 'checked' : '' }}>&nbsp; List &nbsp;
                                                &nbsp;&nbsp;&nbsp;


                                                <input type="checkbox" name="{{ $row . '_add' }}" value="1"
                                                    class="form-check" {{ $role->hasPermissionTo($row . ' add') ? 'checked' : '' }}>&nbsp; Add &nbsp; &nbsp;&nbsp;&nbsp;


                                                <input type="checkbox" name="{{ $row . '_edit' }}" value="1"
                                                    class="form-check" {{ $role->hasPermissionTo($row . ' edit') ? 'checked' : '' }}>&nbsp; Edit &nbsp;
                                                &nbsp;&nbsp;&nbsp;


                                                <input type="checkbox" name="{{ $row . '_delete' }}" value="1"
                                                    class="form-check" {{ $role->hasPermissionTo($row . ' delete') ? 'checked' : '' }}>&nbsp; Delete &nbsp;
                                                &nbsp;&nbsp;&nbsp;


                                                <input type="checkbox" name="{{ $row . '_view' }}" value="1"
                                                    class="form-check" {{ $role->hasPermissionTo($row . ' view') ? 'checked' : '' }}>&nbsp; View &nbsp; &nbsp;&nbsp;&nbsp;
                                            </div>
                                        </div>
                                    </div>
                                @endforeach
